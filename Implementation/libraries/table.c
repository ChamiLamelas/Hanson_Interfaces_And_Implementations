...
static int atom_cmp(const void *key1, const void *key2) { return key1 != key2; }
...
static void auto_rehash(T table) {
  if (table->num_elements < LOAD_FACTOR_THRESHOLD * table->num_buckets) return;
...
void Table_map(T table, void apply(void *key, void **value, void *cl),
               void *cl) {
  assert(table && apply);
