...
static void check_sets_for_set_ops(T s, T t) {
    assert(s || t);
    if (s && t) assert(s->cmp == t->cmp && s->hash == t->hash);
}
...
T Set_union(T s, T t) {
    ...
    T out = Set_new((s ? s->num_elements : 0) + (t ? t->num_elements : 0),
                    s ? s->cmp : t->cmp, s ? s->hash : t->hash);
    struct node *itr;
    if (s) {
...
T Set_inter(T s, T t) {
    check_sets_for_set_ops(s, t);
    if (s == NULL || t == NULL)
        return Set_new(EMPTY_SET_HINT, s ? s->cmp : t->cmp,
                       s ? s->hash : t->hash);
    T out = Set_new(
        s->num_elements < t->num_elements ? s->num_elements : t->num_elements,
        s->cmp, s->hash);
    struct node *itr;
    for (int i = 0; i < s->num_buckets; i++)
        for (itr = s->buckets[i]; itr; itr = itr->next)
...
T Set_minus(T s, T t) {
    check_sets_for_set_ops(s, t);
    if (s == NULL) return Set_new(EMPTY_SET_HINT, t->cmp, t->hash);
...
T Set_diff(T s, T t) {
    ...
    if (s)
        for (int i = 0; i < s->num_buckets; i++)
            for (itr = s->buckets[i]; itr; itr = itr->next)
                if (t == NULL || Set_member(t, itr->member) == 0) {
                    push_node(itr->member,
                              out->buckets + hash(out, itr->member));
                    out->num_elements++;
                }
