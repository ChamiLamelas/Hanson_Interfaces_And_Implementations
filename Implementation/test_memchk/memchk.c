...
static struct node *allocate_node(void *block_ptr, long block_size, const char *file, int line)
{
    if (num_nodes_left == 0)
    {
...
{
    return ((nbytes + sizeof(union align_t) - 1) / sizeof(union align_t)) * sizeof(union align_t);
}

static struct node *find(void *ptr)
{
    struct node *tmp;
    for (tmp = buckets[hash(ptr)]; tmp && tmp->block_ptr != ptr; tmp = tmp->next_bucket)
        ;
    return tmp;
}
...
void *Mem_alloc(long nbytes, const char *file, int line)
{
    ...
    {
        /*
        This statement allows for the same address never being returned twice by Mem_alloc because
        the returned address will always be at least 1 byte away from pointer to beginning of
        free block. Let x be pointer to beginning of block that has been chosen to allocate. y 
        will be the returned pointer. y is at least x + 1. Suppose memory pointed to by y is freed.
        Suppose that the block with y is chosen for allocation, z will be the returned pointer. z
        is at least y + 1. And so on... 
        ...
        If x = y we now inadvertently have x pointing to some other data that is a legitimate
        component of our program and we may do further operations with x and corrupt y's data
        */
        ...
        else if (tmp == &head_free)
        {
            ...
            if (new_block_ptr == NULL)
                Except_raise(&Mem_Failed, file, line);
            struct node *new_block_node = allocate_node(new_block_ptr, new_block_size, file, line);
...
void *Mem_resize(void *ptr, long nbytes, const char *file, int line)
{
    ...
    memcpy(block_ptr, ptr, (nbytes < node->block_size) ? nbytes : node->block_size);
    free_node(node);
    return block_ptr;
}